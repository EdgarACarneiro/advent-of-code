...
def let_sand_fall(
    ...
    if new_pos not in rocks and new_pos not in sand:
        return let_sand_fall(new_pos, rocks, sand, last_rock)
...
def main(input: list[str]) -> int:
    ...
    sand = set()
    while True:
        final_pos = let_sand_fall(INITIAL_COORD, rocks, sand, last_rock)

        if final_pos[1] > last_rock:
            return len(sand)
